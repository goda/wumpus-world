...
from wumpus.src.agent.Agents import NaiveAgent, ProbAgent
...
class WumpusWorld():
    ...
    def main(self, visualize: bool = False):
        def run_episode(env: Environment,
            ...
            if visualize:
                print("Action: ", str(next_action.name), "| Agent Orientation: ",
                      next_environment.agent.orientation.state.name)
        ...
        for i in range(0, 1000):
            ...
            agent = ProbAgent(max_prob_dying_new_loc=0.2)
            total_reward += run_episode(initial_env, agent, initial_percept)
            print("Episode: ", "{:0>3}".format(i+1),
                  ", Total reward: ", "{:0>5}".format(total_reward),)
